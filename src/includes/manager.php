...
class RSSManager extends Ab_ModuleManager {
    ...
    public function DSProcess($name, $rows) {
        ...
        switch ($name) {
            case 'config':
                foreach ($rows->r as $r) {
                    if ($r->f == 'u') {
                        $this->ConfigUpdate($r->d->nm, $r->d->ph);
                    }
                }
                break;
            case 'chanel':
                foreach ($rows->r as $r) {
                    if ($r->f == 'a') {
                        $this->ChanelAppend($r->d);
                    }
                    if ($r->f == 'u') {
                        $this->ChanelUpdate($r->d);
                    }
                    if ($r->f == 'd') {
                        $this->ChanelRemove($r->d->id);
                    }
                }
                break;
            case 'source':
                foreach ($rows->r as $r) {
                    if ($r->f == 'a') {
                        $this->SourceAppend($r->d);
                    }
                    if ($r->f == 'u') {
                        $this->SourceUpdate($r->d);
                    }
                    if ($r->f == 'd') {
                        $this->SourceRemove($r->d->id);
                    }
                }
                break;
        }
    ...
    public function DSGetData($name, $rows) {
        ...
        switch ($name) {
            ...
            case 'config':
                return $this->Config($p->mod);
            case 'chanel':
                return $this->ChanelList();
            case 'source':
                return $this->SourceList();
            case 'chanelsource':
                return $this->ChanelSourceList();
            case 'record':
                return $this->RecordList($p->chanelid);
            case 'online':
                return $this->Online();
        }
